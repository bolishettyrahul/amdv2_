...
# These huge layers come first so dependency/code changes never re-download them.
RUN apt-get update && \
    apt-get install -y --no-install-recommends curl ca-certificates zstd && \
    curl -fsSL https://ollama.com/install.sh | sh && \
    curl -fL --retry 5 --retry-all-errors -C - \
        https://ollama.com/download/ollama-linux-amd64-rocm.tar.zst -o /tmp/ollama-rocm.tar.zst && \
...
# Install the core dependencies plus the spaCy NER model Stage 1 uses to
# resolve NER tasks at $0. click is spelled out because spaCy's typer
# dependency fails to pull it in on this base image, and `import spacy`
# dies without it; the import check makes that failure break the build.
RUN pip install --no-cache-dir --retries 10 --timeout 60 -r requirements.txt click>=8.0 \
